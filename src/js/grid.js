...
export class Grid {
    ...
    constructor(DOM_el) {
        ...
        this.DOM.cells = [...this.DOM.el.querySelectorAll('.grid__cell-item')];
        this.DOM.cells.forEach(el => this.cellArr.push(new Cell(el)));
        this.DOM.content = document.querySelector('.grid-zoom-content');
    ...
    initEvents() {
        ...
        for (const [position,imageCell] of this.imageCellArr.entries()) {
            ...
            imageCell.DOM.el.addEventListener('click', (e) => {
        ...
        this.DOM.miniGrid.cells.forEach((cell, position) => {
            cell.addEventListener('click', () => {
                if (this.isAnimating || this.currentCell === position || !cell.classList.contains('is-image')) {
                    return false;
                }
                this.isAnimating = true;
    ...
    showContent(imageCell) {
        ...
        this.otherImageCells = this.DOM.cells.filter(el => el != imageCell.DOM.el);
        ...
        gsap.timeline({
            ...
            // overflow hidden
            onStart: () => docEl.classList.add('oh'),
            onComplete: () => {
        ...
        .add(() => {
            ...
            this.DOM.content.classList.add('grid-zoom-content--open');
        }, 'showContent')
        .add(() => imageCell.contentItem.DOM.el.classList.add('grid-zoom-content__item--current'), 'showContent+=0.02');
    }
    ...
    closeContent() {
        ...
        gsap.timeline({
            ...
            // overflow hidden
            onStart: () => docEl.classList.remove('oh'),
            onComplete: () => {
                this.isAnimating = false;
            }
        })
        ...
        .add(() => {
            ...
            this.DOM.content.classList.remove('grid-zoom-content--open');
        }, 'start')
        .add(() => imageCell.contentItem.DOM.el.classList.remove('grid-zoom-content__item--current'))
        .addLabel('showGrid', 0)
    ...
    changeContent(position) {
        ...
        .add(() => {
            imageCell.contentItem.DOM.el.classList.remove('grid-zoom-content__item--current');
        })
        ...
        .add(() => {
            upcomingImageCell.contentItem.DOM.el.classList.add('grid-zoom-content__item--current');
        }, 'showContent+=0.02');
    }
    /**
